...
class S2ANetHead_CONTEXT(nn.Module):
    ...
    def __init__(self,
                 ...
                 loss_odm_bbox=dict(
                     type='SmoothL1Loss', beta=1.0 / 9.0, loss_weight=1.0)):
        super(S2ANetHead_CONTEXT, self).__init__()
    ...
    def forward_single(self, x, stride):
        ...
        # add
        # Selection of representative features and representative confidences
        # scores:15 representative features
        # norm_features:15 representative confidences
        # hs ws:position of representative features
        scores=torch.zeros([15]).cuda()
        rep_fam_cls_score=fam_cls_score
        hs=torch.zeros([15]).cuda()
        ws=torch.zeros([15]).cuda()
        features=torch.zeros([15, 256]).cuda()
        for i in range(15):
            if rep_fam_cls_score.sigmoid()[i//15,i%15,:,:].max()>=0.6:
                scores[i]=rep_fam_cls_score[i//15,i%15,:,:].max()
            index=rep_fam_cls_score[i//15,i%15,:,:].argmax()
            h=x.size()[2]
            hs[i]=h
            w=x.size()[3]
            ws[i]=w
            features[i]=x[i//15,:,index//w, index%w]

        norm_features=features.t()
        # /add
        ...
        # sharing the parameters of CLSFAM and CLSODM
        for fam_cls_conv in self.fam_cls_convs:
            odm_cls_feat = fam_cls_conv(odm_cls_feat)
        odm_cls_score = self.fam_cls(odm_cls_feat)

        odm_bbox_pred = self.odm_reg(odm_reg_feat)

        # Confidence increase module for hard positive samples
        if scores.max()!=0:
            norm_or_feat=or_feat.reshape(256,-1).transpose(0, 1)
            # Cosine Similarity
            # norm_or_feat=torch.div(norm_or_feat, torch.sqrt(torch.sum(norm_or_feat**2,dim=1)).repeat(256,1).t())
            
            # Embedded Gaussian Similarity
            # softmax operation of 15 Classes
            similarity=norm_or_feat.mm(norm_features).softmax(dim=1)

            rep_scores=similarity.mul(scores.repeat(h*w,1)).reshape(h, w, 15).permute(2, 0, 1).unsqueeze(0)
            odm_cls_score=odm_cls_score+rep_scores
