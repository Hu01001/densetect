...
def show_result_rbox(img, 
    ...
    for j, name in enumerate(class_names):
        ...
        for det in dets:
            ...
            if show_label:
                y=min([bbox[1],bbox[3],bbox[5],bbox[7]])
                cv2.putText(img, '%.3f' % (score), (bbox[2*[bbox[1],bbox[3],bbox[5],bbox[7]].index(y)],y),
                            color=color, fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=0.5)
    return img